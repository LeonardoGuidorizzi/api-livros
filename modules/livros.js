...
const getLivros = function (keyword){
    let nome = keyword.toLowerCase()
    let containerJson = {}
    let livrosContainer = []
    let totalLivros = 0;

    livros[0].books.forEach(itemLivro => {
        if (itemLivro.title.toLowerCase().includes(nome)) {
            let livrosInfo = {}
    
            livrosInfo.title = itemLivro.title
            livrosInfo.subtitle = itemLivro.subtitle
            livrosInfo.price = itemLivro.price
            livrosInfo.image = itemLivro.image
    
            livrosContainer.push(livrosInfo);
        }
    });
    totalLivros = livrosContainer.length;
    containerJson.total = totalLivros;
    containerJson.livros = livrosContainer;

    return containerJson;
    
}

// let teste = getLivros('php');
// console.log(teste);
/**
 * {
 *  total: xx,
